...
**Migraciones en Docker:** el `ENTRYPOINT` (`docker-entrypoint.sh`) ejecuta antes de Gunicorn: **`migrate`**, **`seed_categorias`**, **`crear_admin`** y **`ensure_demo_seed`**. Con **`DEMO`** activo, si falta Jaime demo, corre **`seed_demo_minimal`** (segundos: familia + 2 usuarios) para que **Render no mate el arranque** esperando un `seed_demo` largo. Los **15 meses de datos** del demo completo van en **Release Command** `./release.sh` (ejecuta **`seed_demo`**) o manualmente con `python manage.py seed_demo`. Variables: **`SKIP_MIGRATE_ON_START=1`**, **`SKIP_POST_MIGRATE_SETUP=1`**. El endpoint **`GET /api/usuarios/config/` no usa la base de datos**: puede dar **200** sin tablas; **`demo-login` 404** suele indicar usuarios demo aún no creados.
