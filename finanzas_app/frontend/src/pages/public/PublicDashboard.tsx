...
import type { MetricasPublicasApi } from '@finanzas/shared/api/finanzas'
...
export default function PublicDashboard() {
  ...
  const tieneMetodoPago =
    metodo_pago != null &&
    (metodo_pago.efectivo > 0 || metodo_pago.debito > 0 || metodo_pago.credito > 0)
  const tieneDesagregaciones =
    gasto_por_categoria.length > 0 ||
    tieneMetodoPago ||
    estacionalidad.length > 0 ||
    presupuesto_vs_real != null
  ...
  return (
    <section className={styles.root}>
      ...
      {!tieneDesagregaciones ? (
        <p className={styles.privacyNote}>
          Las desagregaciones (categorías, métodos de pago, etc.) se publican cuando hay suficientes
          usuarios para preservar el anonimato.
        </p>
      ) : (
