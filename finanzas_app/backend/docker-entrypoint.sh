#!/bin/sh
# Antes de Gunicorn: migraciones + setup rÃ¡pido. seed_demo completo no va en primer plano
...
if [ "${SKIP_POST_MIGRATE_SETUP:-0}" != "1" ]; then
  # Demo primero: solo seed_demo_minimal vÃ­a ensure_demo_seed (segundos) â†’ login usable al abrir el puerto.
  ...
  if [ "${SKIP_BACKGROUND_DEMO_SEED:-0}" != "1" ] && {
    ...
    echo "==> docker-entrypoint: seed_demo_if_empty (segundo plano; histÃ³rico completo si falta)"
